...
#+BEGIN_SRC ruby
  ...
  #run_file "#{base_dir}samples.rb" -- not there anymore
  #run_file "#{base_dir}numark_config-deprecated.rb"
  run_file "#{base_dir}numark_sampler.rb"
  # osc
  ...
  run_file "#{haus_dir}songs/shared.rb"
#+END_SRC
* scratch
** rhythms
...
#+BEGIN_SRC ruby
  ...
  tick(:symbol)
#+END_SRC
** <2018-12-12 Wed> vizualization ideas
particle flow:
https://codepen.io/seanfree/pen/PxJyEW

shape outside polygon:
https://codepen.io/enxaneta/pen/jrpkBG

shooting stars:
https://codepen.io/YusukeNakaya/pen/XyOaBj

generative artistry:
https://codepen.io/collection/XkyjLN/

smoke svg filter:
https://codepen.io/rafaelcastrocouto/pen/rqbovJ

neural beatbox:
https://codepen.io/naotokui/pen/NBzJMW
** <2019-05-01 Wed> work to do on sonic pi

Bug:
- when open prefs it resizes the buffers but doesn't go back
- sometimes the highlight gets stuck -- when moving text up or down... how to repo?
- better debugging loaded files (does not give proper references)
- some way to see what file the error happened in if loaded?
- hitting enter doesn't carry to the proper indentation level

*** <2019-05-13 Mon> in_thread post notes: collection of best practices for efficiency?

- in ableton we just bounce down tracks -> effective workflows for doing this smoothly, on-the fly (recording and storing audio files on the fly)
-



snippets that are non-standard?

having to restart -- a good way of troubleshooting

how to recover fast when accidentally starting a loop out of sync?

tick and look inside of at...
*** introduction:
classify myself as a world percussionist. Most of my professional experience is in Japanese Taiko drumming (taikoproject), which I have been playing for more than a decade,
dabbled with DIY electronic drum kits. These days make a lot of music with Ableton (mostly live jamming, but you can out some early recordings here...)
 Sonic Pi in 2014 or 15.
started using seriously in 2017, started jamming with people, integrating with Ableton in 2017.

** <2019-05-15 Wed> jay bassline
#+BEGIN_SRC ruby
...
#+END_SRC
** <2019-05-15 Wed> deep house synth save
#+BEGIN_SRC ruby

  live_loop :deep_house_bassline, sync: :out do
    #deep_house_bass_pattpfern(
    melody = {
      notes: scale(:D3, :major).pick(5),
      times: ring(0,1,2,3,4,5,6,7),
      durations: ring(0.25, 1).pick(5)
    }

    puts "Melody times: #{melody[:times]}"
    dhb_pattern(melody)

    sleep 8
  end

  # give hash: {  notes: ring(:C,:A,:D...),
  #             times: ring(1,2,1,1,...},
  #             durations: ring(0.5, 0.5, 0.25,...)

  define :dhb_pattern do |melody|
    in_thread do
      at melody[:times] do |time|
        deep_house_bass(melody[:notes].look, melody[:durations].look)
      end
      sleep melody[:times].max
    end
  end
#+END_SRC
** <2019-07-22 Mon> save numark_sampler
#+BEGIN_SRC ruby
  #
  # haustothehaus
  #
  ...
  live_loop :sixteen, sync: :garage do
    sample '/Users/daniel/recording/haus_samples', 'indigo',
      rate: 0.9950629921259843,
      start: 0.049,
      finish: 0.1045
    #bassment
  ...
  live_loop :eight, sync: :garage do
    with_fx :bpf, centre: 40, centre_slide: 4 do |fx|
      in_thread do
        control fx, centre: 120
        sleep 4
        control fx, centre: 80
      end

      4.times do
        sample '/Users/daniel/recording/haus_samples', 'indigo', amp: 0,
          rate: 0.9160629921259843,
          start: 0.148,
          finish: 0.2105
        sleep 2
      end
    ...
    #sample_slicer(haus_samps, 'indigo')

    #sample_slicer(haus_samps, 'indigo')
  end

  live_loop :four, sync: :garage do
    #numark_sampler(haus_samps, 'indigo')
  ...
  live_loop :one, sync: :garage do
    gravel_bd
    #sample_slicer(haus_samps, 'indigo')
    if tick % 2 == 0
      sample haus_samps, 'neu_haus_keys',
        start: 0.276,
        finish: 0.2855
    end
  ...
  ##| end
#+END_SRC
** <2019-10-27 Sun> numark sampler, l/r split progress
#+BEGIN_SRC ruby
  ...
  live_loop :eight do
    numark_sampler_a(haus_samps, 'indigo')
  sample '/Users/daniel/recording/haus_samples', 'indigo', amp: 0,
          rate: 1.2599210498948732,
          start: 0.9725,
          finish: 0.9780118110236221
    puts "play"
    sleep 1
  end
  ...
  define :numark_sampler_a do |collection, sample|
    in_thread do
      ...
      if get(:playing_a)
        ...
        midi_note_on 0,2, channel: 1 # set play button
  ...
  live_loop :clipboard_a do
    use_real_time
    note, val = sync "/midi/dj2go2/1/1/note_on"
...
#+END_SRC
** <2019-11-10 Sun> sorting algorithms

#+BEGIN_SRC ruby
  use_bpm 130

  define :play_arr do |a, synth|
    use_synth synth
    a.each do |n|
      play n,
        attack: 0.1, decay: 0.1, sustain: 0.2, release: 0.1
      sleep 1
    end
  end
  ...
  array_b = scale(:C, :major).shuffle.to_a
  array_m = array_b
  array_i = array_b
  ...
  in_thread do
    #merge_sort(array_m, 0, array_m.length-1)
  end

  in_thread do
    insertion_sort(array_i)
  end


  # element it is acting on, vs whole array it's just taken a step on
...
#+END_SRC
** <2019-11-16 Sat> numark sampler (old?) save
#+BEGIN_SRC ruby
  ...
  define :numark_sampler_b do |collection, sample|
    in_thread do
      ...
      if get(:playing_b)
        ...
        midi_note_on 0,2, channel: 2 # set play button
  ...
  live_loop :clipboard_b do
    use_real_time
    note, val = sync "/midi/dj2go2/1/2/note_on"
...
#+END_SRC
** <2019-11-16 Sat> garage numbered loops experiment
#+BEGIN_SRC # ruby
# haustothehaus
#
...
live_loop :one do
  ...
  if false#tick%8 != 1
    with_fx :reverb, mix: 0.3 do
      sample '/Users/daniel/recording/haus_samples', 'indigo',
        rate: knit(0.25,4,0.5,2,0.72,1).tick(:asdfs),
        start: 0.431,
        finish: 0.4365
    end
  end
...
live_loop :two_garage, sync: :one do
  #numark_sampler(haus_samps, 'canadien_errant')
  sample '/Users/daniel/recording/haus_samples', 'canadien_errant', amp: 0,
    rate: 1.0,
    start: 0.7705,
    finish: 0.8525
  sleep 2
...
live_loop :four_garage, sync: :one do
  ...
  sample '/Users/daniel/recording/haus_samples', 'canadien_errant', amp: 0,
    ...
    start: 0.02,
    finish: 0.1215
  #nm_sampler(haus_samps,
  sleep 0.5
bonnie
  #tile(knit(:D,3,note(:D)+5,1).tick, 0.25)

  sleep 3.5
end

define :bonnie do
      sample '/Users/daniel/recording/haus_samples', 'canadien_errant', amp: 0.75,
    rate: 1.0/(5/4),
    start: 0.6505,
    finish: 0.7375
...
live_loop :eight, sync: :one do
  lightswitch

  if knit(false,3,true,1).tick
    sample '/Users/daniel/recording/haus_samples', 'canadien_errant',
      rate: 1.0,
      start: 0.524,
      finish: 0.591
  end
...
live_loop :sixteen_garage, sync: :one do
  sample haus_samps, 'leaky_door'
...
a triumphant triumverate of minds
** <2019-11-27 Wed> garage jam
#+BEGIN_SRC ruby
  # Standard Incantation
  base_dir = '/Users/daniel/src/talldan_sonicpi/'
  #load_snippets("#{base_dir}snippets/")
  ...
  live_loop :garage do
    #numark_sampler_a(haus_samps, 'alibi')

    unless tick(:counter) % 8 == 0
      with_fx :bpf, centre: range(50,120,10).mirror.tick do
        sample '/Users/daniel/recording/haus_samples', 'alibi', amp: 0.7,
          rate: 1.0,
          start: 0.2195,
          finish: 0.282492125984252
      end
    else
            sample '/Users/daniel/recording/haus_samples', 'alibi', amp: 0.7,
          rate: 1.0,
          start: 0.3195,
          finish: 0.382492125984252
    end


    sleep 1

    sample '/Users/daniel/recording/haus_samples', 'car',
      rate: 2.0,
      start: 0.1655,
      finish: 0.17573622047244095
    sleep 1
  end

  live_loop :eighst, sync: :garage do

    sample '/Users/daniel/recording/haus_samples', 'car',
      rate: 2.0,
      start: 0.0185,
      finish: 0.04527165354330709
  ...
  define :hold do |times, key|
    knit(false,times,true,1).ramp.tick(key)
  end
...
#+BEGIN_SRC ruby
  ...
  # A: path, car (phaser?), keys, gravel_bd, sub_bass :Fs2
  # B: path, haus_garage_door, car_door_close, bushes (echo 0.25)
  ...
  live_loop :eight, sync: :garage do
    #keys
    #car_door_close
    with_fx :echo, amp: range(0.25,1,0.25).ramp.tick, phase: 0.25 do
      #bushes
    end
  ...
  define :gravel_bd do
    with_fx :reverb, mix: 0.2, room: 0.5, damp: 1 do
      with_fx :distortion, distort: 0.3 do
        sample :bd_haus, amp: 0.5, attack: 0.01, sustain: 0.1, release: 0.1
        with_fx :lpf, cutoff: 50 do
          with_fx :octaver, subsub_amp: 1, sub_amp: 1, super_amp: 0 do
            sample :drum_tom_mid_soft, amp: 0.1, start: 0.01,
...
#+END_SRC
** <2020-02-24 Mon> wavetable
#+BEGIN_SRC ruby
  live_loop :pasta do
    use_synth :tri
    with_fx :slicer, phase: 0.5, phase_slide: 4 do |sl|
      with_fx :lpf, cutoff: 50 do
        with_fx :echo, decay: 4 do
          s1 = play :B4, attack: 0.01, decay: 0.25, pan: -1, pitch: -0.05, pitch_slide: 2
          s2 = play :B4, attack: 0.01, decay: 0.25, pan: 1, pitch: 0.05, pitch_slide: 4
          control s1, pitch: 0.1
          control s2, pitch: -0.1
        end
      end
  ...
  live_loop :sauce, sync: :pasta do
    sample :bd_klub

    sleep 1
  end

  live_loop :meat, sync: :pasta do
    sleep 0.5
    sample :elec_blip, sustain: range(0.01,0.05,0.005).choose, cutoff: range(60,120,10).tick

  end
#+END_SRC
** <2020-03-29 Sun> michael music maker first draft
#+BEGIN_SRC ruby
  #loop_maker
  ...
  ##|         # put effects here
  ##|         sample pattern[0], pattern[1], amp: relative_level(num_patterns)
  ##|         #sleep pattern length etc
  ##|         #at
  ##|       end
  ##|     end
  ##|     puts "pattern[2]: #{pattern[2]}"
  ##|     decremented_beats_remaining = pattern[2] - 1
  ##|     # problem: how do I keep track of the sets?
  ##|     unless decremented_beats_remaining <= 0 && pattern[3] > 0
  ##|       new_patterns << [pattern[0], pattern[1], decremented_beats_remaining, pattern[3], pattern[4]]
  ##|     end

  ##|   end
  ##|   set(:drum_patterns, new_patterns)
  ##| end



  ##| live_loop :pinos, sync: :test do
  ##|   use_synth :piano

  ##|   root = chord(:G4, :minor7, invert: 2).tick

  ##|   with_fx :reverb do
  ##|     4.times do |n|
  ##|       chord(root, :minor7).each_with_index do |note, i|

  ##|         play note, amp: 1 unless n%(i+1) == 0
  ##|         sleep 0.25
  ##|       end
  ##|     end
  ##|   end
...
#+END_SRC
** <2020-04-04 Sat> michael music maker

#+BEGIN_SRC ruby
  use_bpm 120

  #use_random
  #set(:playing_patterns, [])# if get(:playing_patterns).nil?

  live_loop :conductoratorz do
    sample :elec_beep

    #loop_maker
    puts "pp: #{get(:playing_patterns)}"
    drum_loop_maker
  ...
  define :drum_loop_maker do# |loop_name|
    ...
    instrument = samples.keys.choose
    sample_name = samples[instrument].choose

    pattern = drum_pattern
    play_at = pattern[:play_times]
    pattern_length = pattern[:length]

    loop_name = random_name
    stop_after = 4

    # pp = get(:playing_patterns).dup

    #return if pp.include?(instrument)
    #pp << instrument
    #set(:playing_patterns, pp)

    live_loop loop_name do
      unless  tick < stop_after
        #remove from buffer
        #pp = get(:playing_patterns).dup
        #pp.delete(instrument)
        # set(:playing_patterns, pp)
        stop
      end
      puts "playing: #{loop_name}"
      at play_at do
        sample sample_name
      end
      sleep pattern_length
    end
  end

  define :drum_pattern do
    building_blocks = [
      #{length: 1, play_times: [0]},
      #{length: 2, play_times: [0,1]},
      {length: 4, play_times: range(0,4,1)},
      {length: 4, play_times: range(0,4,0.5)},
      {length: 4, play_times: spread_to_pattern(spread(3,8))}
    ]

    spread_possibilities = range(0,16,1).map { |n| range(0,24,1).map { |m| [n,m] } }.flatten(1)

    spread_possibilities.each do |sp|
      building_block = {length: sp[1], play_times: spread(sp[0],sp[1])}
      building_blocks << building_block
    end


    building_blocks.choose
    #assemble building blocks, favour symmetry
  end

  define :spread_to_pattern do |spread, step_length = 1|
    spread(3,8).to_a.inject([]) { |acc, bool|
      if bool
  ...
  define :ambi_loop_maker do

  end

  define :bass_loop_maker do
  ...
  define :relative_level do |source_count|
    1/(0.5*Math.log(source_count) + 1)
  end

  define :random_name do
    current_time.to_f.to_s.chars.each_slice(2).map { |tdn| (tdn.join.to_i % 26 + 65).chr }.join.to_sym
  end
#+END_SRC
** <2020-04-18 Sat> michael music maker
#+BEGIN_SRC ruby
  ...
  with_fx :reverb, room: 1 do
    live_loop :conductorator do
      ...
      #set(:kick_pattern, {steps: 16, step_length: 0.5, num_hits: range(0,16).choose}) if one_in(8)
      #set(:kick_pattern, nil)

      use_synth :fm
      8.times do
        play knit(:C,4,:D,7,:E,5).tick
        sleep 0.5
      end
    ...
    live_loop :kick_player, sync: :conductorator do
      ...
      stop if pattern.nil?
      num_hits = pattern[:num_hits]
      steps = pattern[:steps]
      step_length = pattern[:step_length]
      length = steps * step_length

      at spread_to_pattern(spread(num_hits,steps),step_length) do
        kick
      end
      sleep length
    end
  ...
  define :random_drum_pattern do
    puts drum_patterns.length
    drum_patterns
    #assemble building blocks, favour symmetry
  end
  ...
  define :random_name do
    use_random_seed current_time.to_f
    rand.to_f.to_s.chars.shuffle.each_slice(2).map { |tdn| (tdn.join.to_i % 26 + 65).chr }.join.to_sym
  end
...
#+BEGIN_SRC ruby
  ...
  samples_path = '/Users/daniel/recording/samples'
  kick_lab = "#{samples_path}/kick_lab/samples"

  use_bpm 125

  live_loop :zamel_player do
    #play :C, amp: 0.5
    tick

    ##| camel_noise if look % 32 == 0
    sample kick_lab, "deep"

    with_fx :lpf, cutoff: range(50,100).ramp.tick(:asdf) do
      ##| i_love_to_sing if look % 4 == 0
    end
    ...
    numark_sampler_a(zamel_twinkle)


    ##| hi_zamel if look % 8 == 4
    ##| so_lets_all_say if look % 8 == 0
  ...
  define :z do
    in_thread do
      8.times do |n|
        if n % 8 == 0
          sample '/Users/daniel/recording/samples', 'zamel_humps_full.aif',
            rate: 1.0,
            start: 0,
            finish: 0.003937007874015748
        end
        sleep 0.5
        sample '/Users/daniel/recording/samples', 0,
          rate: 1.0,
          start: 0,
          finish: 0.0031496062992125984
        sleep 0.5
      end
    end
  end
  ...
  define :amazameling do
    sample '/Users/daniel/recording/samples', 'zamel_twinkle_full.aif',
      rate: 1.0,
      start: 0.9175,
      finish: 0.9222244094488189
  end


  define :hi_zamel do
    with_fx :bpf, cutoff: 110, res: 0.01 do
      sample '/Users/daniel/recording/samples', 'zamel_twinkle_full.aif',
        rate: 1.0,
        start: 0.1045,
        finish: 0.11709842519685039
    end
  end


  define :so_lets_all_say do

    sample '/Users/daniel/recording/samples', 'zamel_twinkle_full.aif',
      rate: 1.0,
      start: 0.0865,
      finish: 0.10303543307086614
  end


  define :i_love_to_sing do
    sample '/Users/daniel/recording/samples', 'zamel_twinkle_full.aif',
      rate: 1.0,
      start: 0.0445,
      finish: 0.05788582677165354
  end

  define :eye do
    with_fx :bpf, centre: 90, res: 0.01 do
      sample '/Users/daniel/recording/samples', 'zamel_twinkle_full.aif',
        cutoff: 100,
        amp: 0.8,
        rate: 1,
  ...
  define :twinkle do
    with_fx :hpf, cutoff: 100 do
      ...
      sample '/Users/daniel/recording/samples', 'zamel_twinkle_full.aif',
        ...
        rate: 1.0,
        start: 0,
        finish: 0.014960629921259844
    end
  end

  define :camel_noise do
    with_fx :echo, phase: 1, decay: 8 do
      sample '/Users/daniel/recording/samples','zamel_twinkle_full.aif',
        amp: 0.5,
        rate: 1.0,
        start: 0.123,
        finish: 0.13087401574803148
    end

  end

  define :ra do
    sample '/Users/daniel/recording/samples', 'zamel_humps_full.aif', cutoff: 100,
      rate: 1.0,
      start: 0.1145,
      finish: 0.11843700787401576
  end
  ...
  define :zamel_humps do
    {
      pack: samples_path,
      name: 'zamel_humps_full.aif',
      default_start: 0,
      cue_points: [
        0,
        0.0055,
  ...
  define :zamel_twinkle do
    {
      pack: samples_path,
      name: 'zamel_twinkle_full.aif',
      default_start: 0,
      cue_points: [
        0,
        0.0055,
...
#+END_SRC
** <2020-07-10 Fri> sonic pi idioms

#+BEGIN_SRC ruby
  # if knit(true, 4, false, 4)[look]
  # if knit(true, 1, false, 1)[look]

  # if look % 8 == 0

#+END_SRC
** <2020-07-14 Tue> potentially useful methods
#+BEGIN_SRC
